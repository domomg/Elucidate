...
namespace GUIUtils
{
   public partial class DriveSpaceDisplay : UserControl
   {
      ...
      private void FindAndAddDisplaySizes(BackgroundWorker worker, string path)
      {
         ...
         FreeBytesAvailable(out freeBytesAvailable, path, out pathUsedBytes, out rootBytesNotCoveredByPath);
         ...
         out ulong rootBytesNotCoveredByPath)
      {
         ...
         GetDiskFreeSpaceExW(di.Root.FullName, out freeBytesAvailable, out totalBytes, out num3);
