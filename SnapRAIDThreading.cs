   ...
   public sealed partial class Elucidate
   {
      ...
      private void actionWorker_DoWork(object sender, DoWorkEventArgs e)
      {
         try
         {
            ...
            mreErrorDone.Reset();
            string appPath;
            string args = FormatSnapRaidCommandArgs(command, out appPath);
            if (runWithoutCaptureMenuItem.Checked)
            {
               Log.Warn("Running without Capture mode");

               var fileTarget = (FileTarget)((AsyncTargetWrapper)LogManager.Configuration.FindTargetByName("file")).WrappedTarget;
         ...
         {
            Log.ErrorException("Thread closing abnormally", ex);
            throw;
         }
         finally
         {
      ...
      private void ReadStandardError(ThreadObject threadObject)
      {
         ...
         {
            Log.ErrorException("ReadStandardError: ", ex);
         }
         mreErrorDone.Set();
      }
      ...
      private void ReadStandardOutput(ThreadObject threadObject)
      {
            ...
            do
            {
               if (!string.IsNullOrEmpty(buf = threadObject.cmdProcess.StandardOutput.ReadLine()))
               {
                  ...
                  if (buf.Contains("%"))
                  {
                     ...
                     int percentProgress;
                     if( int.TryParse(splits[0], out percentProgress))
                     {
         ...
         {
            Log.ErrorException("ReadStandardOutput: ", ex);
         }
         mreOutputDone.Set();
      }
      ...
      private void actionWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
      {
         ...
         else if (e.Error != null)
         {
            ...
            Log.ErrorException("Thread threw: ", e.Error);
            comboBox1.Text = "Abort";
         }
         else
         {
